...
// 每一次写操作都应该向这个文件写入若干个字节的数据，作为一个独立的数据库存在，这意味着写操作之间不能彼此干扰，写入的内容之间也不能出现穿插和混淆的情况。
// 每一次读操作都应该从这个文件中读取一个独立完整的数据块。它们读取的数据块不能重复，且需要按顺序读取。
// 例如：第一个读操作读取了数据块1,第二个操作就应该读取数据块2,第三个读操作则应该读取数据库3,以此类推
...
// 获取并更新读偏移量，根据读偏移量从文件中读取一块数据，把该数据块封装成一个Data类型值并将其作为结果返回
func (df *myDataFile) Read() (rsn int64, d Data, err error) {
	...
	for {
		...
		if err != nil {
			// 由于进行写操作的Goroutine比进行读操作的Goroutine少，所以过不了多久读偏移量roffset的值就会大于偏移量woffset的值
			// 也就是说，读操作很快就没有数据块可读了，这种情况会让df.f.ReadAt方法返回的第二个结果值为代表的非nil且会与
			// io.EOF相等的值。因此不应该把EOF看成错误的边界情况
			// so 在读操作读完数据块，EOF时解锁读操作，并继续循环，尝试获取同一个数据块，直到获取成功为止。
			if err == io.EOF {
				//注意，如果在该for代码块执行期间，一直让读写锁fmutex处于读锁定状态，那么针对它的写操作将永远不会成功
				//且相应的Goroutine也会被一直阻塞，因为它们是互斥的
				//so 在每条return & continue 语句的前面加入一个对该读写锁的读解锁操作
				df.fmutex.RUnlock()
