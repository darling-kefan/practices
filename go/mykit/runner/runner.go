// Pakcage runner provide support for writing tasks that must complete
...
func (r *Runner) CheckShutdown() bool {
	select {
	case <-r.shutdown:
		// We have asked to shutdown
		return true
	default:
		// We have not asked to shutdown.
		return false
	}
}
...
func (r *Runner) processor(traceID string, job Jobber) {
	...
	// Defer the send on channel so it happens
